...
const App: React.FC = () => {
  ...
  // --- EXPORT/IMPORT ---
  const handleExport = () => {
    const data: TournamentSession = { players, rounds, currentRoundIndex, courtCount, numRounds, selectedDuration, view, timestamp: new Date().toISOString() };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url; link.download = `pickleflow-${new Date().toISOString().split('T')[0]}.json`;
  ...
  const generateSchedule = () => {
    ...
    players.forEach(p => { playCount[p.id] = 0; players.forEach(p2 => { if (p.id !== p2.id) partnerHistory[`${Math.min(p.id, p2.id)}-${Math.max(p.id, p2.id)}`] = 0; })});
    ...
    for (let r = 0; r < numRounds; r++) {
      ...
      while (available.length >= 4) {
        ...
        const pairings = [ { t1: [group[0], group[1]], t2: [group[2], group[3]] }, { t1: [group[0], group[2]], t2: [group[1], group[3]] }, { t1: [group[0], group[3]], t2: [group[1], group[2]] } ].sort((a, b) => {
          ...
          {rounds.length > 0 && (
            <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border">
              {[ { id: 'setup', icon: Settings, label: 'Setup' }, { id: 'play', icon: PlayCircle, label: 'Play' }, { id: 'summary', icon: LayoutGrid, label: 'Schedule' }, { id: 'leaderboard', icon: Trophy, label: 'Stats' } ].map(item => (
                <button key={item.id} onClick={() => setView(item.id as View)} className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-[10px] font-black transition-all ${view === item.id ? 'bg-white dark:bg-slate-700 text-lime-600 shadow-sm' : 'text-slate-500'}`}><item.icon size={16} /> <span className="uppercase">{item.label}</span></button>
              ))}
            </div>
          )}
        </div>
      ...
      <main className="max-w-4xl mx-auto px-4 mt-6">
        {view === 'setup' && (
          <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border text-center font-black text-lime-600"><h3 className="text-[10px] text-slate-400 uppercase">Rounds</h3><select value={numRounds} onChange={(e) => setNumRounds(parseInt(e.target.value))} className="w-full bg-transparent">{ROUND_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}</select></div>
              <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border text-center font-black text-lime-600"><h3 className="text-[10px] text-slate-400 uppercase">Min</h3><select value={selectedDuration} onChange={(e) => {setSelectedDuration(parseInt(e.target.value)); setTimeLeft(parseInt(e.target.value)*60);}} className="w-full bg-transparent">{DURATION_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}</select></div>
              <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border text-center font-black text-lime-600"><h3 className="text-[10px] text-slate-400 uppercase">Courts</h3><select value={courtCount} onChange={(e) => setCourtCount(parseInt(e.target.value))} className="w-full bg-transparent">{COURT_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}</select></div>
            </div>
            <button onClick={generateSchedule} disabled={players.length < 4} className="w-full py-6 rounded-2xl font-black text-2xl uppercase italic bg-lime-600 text-white shadow-xl disabled:bg-slate-200 disabled:text-slate-400">Start Tournament</button>
          </div>
        )}
        ...
        {view === 'play' && rounds[currentRoundIndex] && (
          <div className="max-w-2xl mx-auto space-y-6 pb-24">
             <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border shadow-md flex items-center justify-between">
                ...
                <div className="text-center"><p className="text-2xl font-black uppercase italic">Round {currentRoundIndex + 1}</p>
                <div className="flex items-center justify-center gap-3 mt-1"><button onClick={() => setTimeLeft(selectedDuration * 60)} className="text-slate-400"><RotateCcw size={14}/></button><span className="font-mono font-black text-lime-600 text-lg">{formatTime(timeLeft)}</span><button onClick={toggleTimer} className="text-slate-400">{timerActive ? <Pause size={14} fill="currentColor"/> : <Play size={14} fill="currentColor"/>}</button></div></div>
             ...
             <div className="space-y-6">
                {rounds[currentRoundIndex].matches.map((match) => (
                  <Card key={match.id} className={`${match.completed ? 'opacity-60' : 'border-l-8 border-lime-500 shadow-lg'}`}>
                    <div className="p-4 space-y-4">
                      <div className="flex justify-between items-center bg-lime-50/50 p-3 rounded-xl border">
                        <div className="font-black uppercase text-sm">{match.team1[0]?.name}<br/>{match.team1[1]?.name}</div>
                        <input type="tel" value={match.score1} onChange={(e) => updateScore(match.id, 1, e.target.value)} className="w-16 h-16 bg-white rounded-xl text-center text-3xl font-black border-2 border-lime-200" />
                      </div>
                      <div className="flex justify-between items-center bg-blue-50/50 p-3 rounded-xl border">
                        <div className="font-black uppercase text-sm">{match.team2[0]?.name}<br/>{match.team2[1]?.name}</div>
                        <input type="tel" value={match.score2} onChange={(e) => updateScore(match.id, 2, e.target.value)} className="w-16 h-16 bg-white rounded-xl text-center text-3xl font-black border-2 border-blue-200" />
                      </div>
                      <button onClick={() => setRounds(prev => prev.map(r => ({...r, matches: r.matches.map(m => m.id === match.id ? {...m, completed: !m.completed} : m)})))} className="w-full py-4 rounded-xl font-black text-xs bg-lime-600 text-white uppercase">{match.completed ? 'Edit Score' : 'Add Final Score'}</button>
                    </div>
                  </Card>
                ))}
             </div>
        ...
        {view === 'summary' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pb-24">
            {rounds.map((round, rIdx) => (
              <Card key={rIdx} className={`p-4 ${rIdx === trueActiveRoundIndex ? 'ring-4 ring-lime-500 scale-[1.02] shadow-2xl bg-white' : 'opacity-60'}`}>
                <h3 className="font-black uppercase italic mb-4">Round {round.number}</h3>
                <div className="space-y-4">
                  {round.matches.map((m, mIdx) => (
                    <div key={mIdx} className="p-3 rounded-xl border border-slate-100 bg-slate-50">
                      <div className="flex justify-between text-[8px] font-bold text-slate-400 mb-2"><span>Court {m.court}</span>{m.completed && <span className="text-lime-600">Final: {m.score1}-{m.score2}</span>}</div>
                      <p className="text-[11px] font-black uppercase">{m.team1.map(p => p.name).join(' & ')}</p>
                      <div className="h-[1px] bg-slate-200 my-1" />
                      <p className="text-[11px] font-black uppercase">{m.team2.map(p => p.name).join(' & ')}</p>
                    </div>
                  ))}
                  {round.sittingOut.length > 0 && <div className="mt-2 text-[10px] font-bold text-orange-600">Resting: {round.sittingOut.map(p => p.name).join(', ')}</div>}
                </div>
              </Card>
            ))}
          </div>
        ...
        {view === 'leaderboard' && (
          <div className="max-w-3xl mx-auto space-y-6 pb-24">
            <div className="flex justify-between items-end">
              <div><h2 className="text-4xl font-black italic uppercase">Standings</h2><button onClick={() => setShowInfo(true)} className="mt-2 flex items-center gap-1.5 text-lime-600 font-black text-[10px] uppercase"><Info size={14}/> How scoring works</button></div>
              <div className="flex bg-slate-100 p-1 rounded-xl border">
                <button onClick={() => setSortKey('avgPoints')} className={`px-4 py-2 rounded-lg text-[10px] font-black ${sortKey === 'avgPoints' ? 'bg-white text-lime-600 shadow-sm' : 'text-slate-500'}`}>Avg PPG</button>
                <button onClick={() => setSortKey('pointsFor')} className={`px-4 py-2 rounded-lg text-[10px] font-black ${sortKey === 'pointsFor' ? 'bg-white text-lime-600 shadow-sm' : 'text-slate-500'}`}>Total Pts</button>
              </div>
            </div>
            <div className="grid grid-cols-1 gap-3">
              {leaderboard.map((stat, idx) => (
                <div key={stat.id} className="flex items-center gap-4 p-4 rounded-2xl bg-white border-2">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-xl ${idx < 3 ? 'bg-lime-500 text-white' : 'bg-slate-100'}`}>{stat.displayRank}</div>
                  <div className="flex-1 min-w-0"><h4 className="font-black text-lg uppercase truncate">{stat.name}</h4><p className="text-[10px] font-bold text-slate-400">W-L: {stat.wins}-{stat.losses} • {stat.gamesPlayed} Games</p></div>
                  <div className="flex gap-4 items-center">
                    <div className={`text-right ${sortKey === 'pointsFor' ? 'opacity-100' : 'opacity-40'}`}><div className="text-lg font-black">{stat.pointsFor}</div><p className="text-[8px] font-black text-slate-400 uppercase">Total</p></div>
                    <div className={`text-right ${sortKey === 'avgPoints' ? 'opacity-100' : 'opacity-40'}`}><div className="text-lg font-black text-lime-600">{stat.avgPoints.toFixed(1)}</div><p className="text-[8px] font-black text-slate-400 uppercase">PPG</p></div>
                  </div>
                </div>
              ))}
            </div>
        ...
        {showTimeUp && (
          <div className="fixed inset-0 z-[200] bg-rose-600/95 backdrop-blur-xl flex items-center justify-center p-4">
             <div className="text-center animate-in zoom-in-95"><Bell size={100} className="text-white mx-auto mb-8 animate-bounce" /><h2 className="text-5xl font-black italic uppercase text-white tracking-tighter mb-8">Time's Up!</h2><button onClick={() => setShowTimeUp(false)} className="px-12 py-6 bg-white text-rose-600 rounded-[2rem] font-black text-2xl uppercase">Dismiss</button></div>
          </div>
        )}
      </main>
    </div>
